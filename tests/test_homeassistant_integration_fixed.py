...
@pytest.mark.asyncio
async def test_integration_setup_and_config_entry(hass: HomeAssistant):
...
@pytest.mark.asyncio
async def test_config_entry_options_update(hass: HomeAssistant):
    ...
    # Update options via config_entries interface
    new_options = {CONF_FEATURE_UPDATE_CHECK: True, CONF_UPDATE_CHECK_TIME: "14:30"}

    # Update the config entry options using the Home Assistant core interface
...
@pytest.mark.asyncio
async def test_config_entry_unload_and_reload(hass: HomeAssistant):
...
@pytest.mark.asyncio
async def test_integration_with_invalid_config(hass: HomeAssistant):
