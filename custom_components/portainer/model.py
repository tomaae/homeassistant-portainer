...
from .controller import PortainerControllerData
...
async def model_async_setup_entry(
    ...
    portainer_controller = hass.data[DOMAIN][config_entry.entry_id]
    sensors = {}
    ...
    portainer_controller.listeners.append(
        async_dispatcher_connect(
            hass, portainer_controller.signal_update, update_controller
        )
    )
    update_controller()
...
def model_update_items(
    inst: str,
    portainer_controller: PortainerControllerData,
    async_add_entities: AddEntitiesCallback,
    ...
    def _register_entity(_sensors, _item_id, _uid, _uid_sensor):
        ...
        return dispatcher[_uid_sensor.func](
            inst=inst,
            uid=_uid,
            portainer_controller=portainer_controller,
    ...
    for sensor in sensor_types:
        ...
        if not uid_sensor.data_reference:
            ...
            if (
                ...
                not in portainer_controller.data[uid_sensor.data_path]
                or portainer_controller.data[uid_sensor.data_path][
                    uid_sensor.data_attribute
                ]
                == "unknown"
            ):
            ...
            for uid in portainer_controller.data[uid_sensor.data_path]:
                uid_data = portainer_controller.data[uid_sensor.data_path]
...
class PortainerEntity:
    ...
    def __init__(
        ...
        self._inst = inst
        self._ctrl = portainer_controller
        self._attr_extra_state_attributes = {ATTR_ATTRIBUTION: ATTRIBUTION}
        self._uid = uid
        if self._uid:
            self._data = portainer_controller.data[self.entity_description.data_path][
                self._uid
            ]
        else:
            self._data = portainer_controller.data[self.entity_description.data_path]
